...
export default function Specs() {
  ...
  return (
    <section id="guitar" className="py-16 sm:py-20 px-4 sm:px-8 max-w-5xl mx-auto border-t-hairline">
      <div className="grid grid-cols-1 md:grid-cols-12 gap-10 items-start">
        ...
        <div className="md:col-span-7 bg-card-bg border-hairline rounded-xl p-5">
          <div className="divide-y divide-border">
            {specs.map((item, idx) => (
              <div key={idx} className="py-2.5 flex flex-col sm:flex-row sm:items-baseline justify-between gap-1 text-xs">
                <span className="text-text-muted sm:w-40 shrink-0 font-medium">{item.label}</span>
                <span className="text-text font-semibold sm:text-right">{item.value}</span>
              </div>
            ))}
          </div>
        </div>
